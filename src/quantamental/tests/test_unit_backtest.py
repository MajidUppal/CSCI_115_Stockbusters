...
class TestCreateAgentOutputFiles:
    ...
    @patch('hybrid_scoring.calculate_backtest_metrics')  # Fixed: patch from hybrid_scoring
    def test_creates_three_files(self, mock_metrics, mock_scores, mock_gcs, 
                                  sample_config, sample_prediction_data, temp_dir):
    ...
    def test_combined_file_exists(self, mock_metrics, mock_scores, mock_gcs,
                                   sample_config, sample_prediction_data, temp_dir):
    ...
    @patch('hybrid_scoring.calculate_hybrid_scores')
    @patch('hybrid_scoring.calculate_backtest_metrics')
    def test_combined_file_has_required_columns(self, mock_metrics, mock_scores, mock_gcs,
                                                 sample_config, sample_prediction_data, temp_dir):
        ...
        required = ['symbol', 'pred_prob_next_month', 'signal', 'Hybrid_Score',
                   'sector', 'industry']
    ...
    @patch('hybrid_scoring.calculate_hybrid_scores')
    @patch('hybrid_scoring.calculate_backtest_metrics')
    def test_creates_company_profiles(self, mock_metrics, mock_scores, mock_gcs,
                                      sample_config, sample_prediction_data, temp_dir):
    ...
    @patch('hybrid_scoring.calculate_hybrid_scores')
    @patch('hybrid_scoring.calculate_backtest_metrics')
    def test_creates_equity_curves(self, mock_metrics, mock_scores, mock_gcs,
                                   sample_config, sample_prediction_data, temp_dir):
...
class TestRunBacktest:
    ...
    def test_run_backtest_returns_results(self, mock_wandb, mock_predictor, mock_gcs,
                                          sample_config, sample_prediction_data):
        """Test run_backtest returns results dict"""
        from backtest import QuantamentalBacktester
        
        # Mock predictor
        mock_pred_instance = Mock()
        mock_pred_instance.predict_next_month.return_value = (sample_prediction_data, None)
        ...
        backtester.create_agent_output_files = Mock(return_value={
            'combined': 'test.csv',
        ...
        result = backtester.run_backtest(sample_prediction_data, use_wandb_logging=False)
