...
def sample_data_files(temp_data_dir):
    ...
    output.to_csv(f"{temp_data_dir}/combined_quantamental_hybrid_with_factors_and_backtest.csv", index=False)
    
    profiles = pd.DataFrame({
        'symbol': ['AAPL', 'GOOGL'],
        'companyName': ['Apple Inc', 'Alphabet Inc']
    })
...
class TestVersioning:
    ...
    def test_create_version_snapshot_structure(self, mock_wandb, sample_config, sample_data_files):
    ...
    @patch('data_versioning.wandb')
    def test_creates_wandb_artifacts(self, mock_wandb, sample_config, sample_data_files):
    ...
    def test_saves_local_snapshot(self, mock_wandb, sample_config, sample_data_files):
        ...
        json_file = Path(sample_config['data']['data_dir']) / 'version_info_test_v1.json'
        assert json_file.exists()
        
    @patch('data_versioning.wandb')
    def test_handles_missing_files_gracefully(self, mock_wandb, sample_config, temp_data_dir):
