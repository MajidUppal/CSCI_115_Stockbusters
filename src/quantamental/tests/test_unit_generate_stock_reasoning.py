...
class TestGetRagConnection:
    ...
    def test_get_rag_connection_success(self, mock_tempdir, mock_chroma_client, mock_storage_client):
...
class TestGetChromaDb:
    ...
    def test_get_chroma_db_returns_query_function(self, mock_embedder, mock_rag_conn):
        ...
        generate_stock_reasoning._cache['default'] = (mock_chroma_client, 'test_collection')
    ...
    @patch('generate_stock_reasoning.get_rag_connection')
    @patch('generate_stock_reasoning.get_embedder')
    def test_get_chroma_db_calls_rag_connection_if_needed(self, mock_embedder, mock_rag_conn):
...
class TestQueryRagTexts:
    ...
    def test_query_rag_texts_returns_document_texts(self, mock_get_chroma_db, mock_rag_conn):
    ...
    @patch('generate_stock_reasoning.get_rag_connection')
    @patch('generate_stock_reasoning.get_chroma_db')
    def test_query_rag_texts_handles_empty_results(self, mock_get_chroma_db, mock_rag_conn):
...
class TestSetupCredentials:
    ...
    @patch('generate_stock_reasoning.service_account.Credentials.from_service_account_file')
...
class TestListGcsFiles:
    ...
    @pytest.mark.unit
    @patch('generate_stock_reasoning.storage.Client', side_effect=Exception("GCS error"))
...
class TestProcessCsvWithRag:
    ...
    def test_process_csv_with_rag_adds_reasoning_column(self, mock_executor):
        ...
        mock_executor_instance.__enter__.return_value.submit = mock_executor_instance.submit
        
        # Mock as_completed
        with patch('generate_stock_reasoning.as_completed', return_value=[mock_future, mock_future2]):
...
class TestAddReasoningToCombinedFile:
    ...
    def test_add_reasoning_to_combined_file_success(self, mock_upload, mock_generate, 
                                                      mock_setup_rag, mock_setup_creds):
        ...
        try:
            ...
            enhanced_df = pd.DataFrame({'symbol': ['AAPL'], 'value': [100], 'rag_reasoning': ['Good']})
            mock_generate.return_value = enhanced_df
            
            result_path = add_reasoning_to_combined_file(
                temp_path,
                upload_to_gcs=False
            )
    ...
    @patch('generate_stock_reasoning.setup_rag_system')
    @patch('generate_stock_reasoning.generate_reasoning_for_dataframe')
    def test_add_reasoning_to_combined_file_handles_existing_column(self, mock_generate,
                                                                      mock_setup_rag, mock_setup_creds):
