...
class QuantamentalBacktester:
    ...
    def run_backtest(self, df: pd.DataFrame, use_wandb_logging: bool = True) -> dict:
        ...
        for file_type, local_path in output_files.items():
            try:
                gcs_path = f"{self.output_folder}/{file_type}_{get_timestamp_suffix()}.csv"
                success = self.gcs.upload_file(local_path, gcs_path)
                
                if success:
                    gcs_paths[file_type] = gcs_path
                    logger.info(f"    {file_type}: gs://{self.gcs.bucket_name}/{gcs_path}")
                else:
        ...
        results = {
            ...
            'gcs_paths': gcs_paths,
            'num_stocks': len(df_predict['symbol'].unique()) if 'symbol' in df_predict.columns else 0,
            'timestamp': datetime.now().isoformat()
        }
    ...
    def create_agent_output_files(self, df_predict: pd.DataFrame) -> dict:
        ...
        logger.info(f"    Input: {len(df_predict):,} rows × {len(df_predict.columns)} columns")
        ...
        if 'fwd_return_1m' not in df_combined.columns:
            ...
            df_combined['fwd_return_1m'] = (
                df_combined.groupby('symbol')['return_1m']
                .shift(-1)
            )
            
            # Calculate forward S&P500 return if sp500_return_1m exists
            if 'sp500_return_1m' in df_combined.columns:
                df_combined['fwd_sp500_return_1m'] = (
                    df_combined.groupby('symbol')['sp500_return_1m']
                    .shift(-1)
                )
        ...
        if 'pred_prob' in df_combined.columns and 'pred_prob_next_month' not in df_combined.columns:
        ...
        if 'signal' not in df_combined.columns and 'pred_prob_next_month' in df_combined.columns:
            prod_threshold = 0.50
            df_combined['signal'] = np.where(
                df_combined['pred_prob_next_month'] > prod_threshold, "LONG-Outperform",
                np.where(df_combined['pred_prob_next_month'] < (prod_threshold - 0.05), 
                         "SHORT-Underperform", "NEUTRAL")
            )
        ...
        if 'sector' not in df_combined.columns or 'industry' not in df_combined.columns:
            ...
            try:
                ...
                if 'sector' not in df_combined.columns and 'sector' in profiles.columns:
                    ...
                    df_combined['sector'] = df_combined['symbol'].map(sector_map).fillna('Unknown')
                    logger.info(f"    Added 'sector' from company profiles")
                
                if 'industry' not in df_combined.columns and 'industry' in profiles.columns:
                    industry_map = profiles.set_index('symbol')['industry']
                    df_combined['industry'] = df_combined['symbol'].map(industry_map).fillna('Unknown')
        ...
        logger.info(f"    Output: {len(df_output):,} rows × {len(df_output.columns)} columns")
        logger.info(f"    Selected {len(final_columns)}/{len(required_columns)} required columns")
        
        if missing_columns:
            logger.error(f"    Missing {len(missing_columns)} columns: {missing_columns}")
        ...
        logger.info(f"      Columns: {len(df_output.columns)} {' CORRECT!' if len(df_output.columns) == 40 else '⚠️ WRONG!'}")
        
        # Verify column count
        if len(df_output.columns) == 40:
            logger.info(f"    SUCCESS: Output has exactly 40 columns as required!")
        else:
            logger.warning(f"     WARNING: Output has {len(df_output.columns)} columns, expected 40!")
        ...
        try:
            ...
            logger.info(f"    Loaded cached company profiles ({len(profiles)} companies)")
        except:
            ...
            if 'sector' in df_output.columns:
                ...
                profiles['sector'] = profiles['symbol'].map(sector_map).fillna('Unknown')
            else:
                profiles['sector'] = 'Unknown'
            
            if 'industry' in df_output.columns:
                industry_map = df_output.groupby('symbol')['industry'].first()
                profiles['industry'] = profiles['symbol'].map(industry_map).fillna('Unknown')
            else:
                profiles['industry'] = 'Unknown'
            
            profiles['description'] = profiles['companyName'] + ' company'
            profiles.to_csv(profiles_path, index=False)
            logger.info(f"     Created minimal company profiles ({len(profiles)} companies)")
        ...
        # ============================================
        equity_data = []
        symbols = df_predict['symbol'].unique()[:100] if 'symbol' in df_predict.columns else []
        
        for symbol in symbols:
            df_sym = df_predict[df_predict['symbol'] == symbol].sort_values('date') if 'date' in df_predict.columns else df_predict[df_predict['symbol'] == symbol]
            
            if 'return_1m' in df_sym.columns:
                df_sym = df_sym.copy()
                df_sym['cumulative_return'] = (1 + df_sym['return_1m'].fillna(0)).cumprod()
            else:
                df_sym = df_sym.copy()
        ...
        logger.info(f"   1. Combined CSV: {len(df_output):,} rows × {len(df_output.columns)} columns")
        logger.info(f"      Target: 40 columns")
        logger.info(f"      Status: {' PERFECT MATCH!' if len(df_output.columns) == 40 else '⚠️ COUNT MISMATCH'}")
        ...
        logger.info(f"      Hybrid Scores (6): Hybrid_Score, Fundamental_Score, Technical_Score, etc.")
