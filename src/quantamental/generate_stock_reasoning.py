...
def process_csv_with_rag(df: pd.DataFrame, chain, sample_size: Optional[int] = None, max_workers: int = 10, main_pbar=None):
...
def generate_reasoning_for_dataframe(df: pd.DataFrame, chain, sample_size: Optional[int] = None, max_workers: int = 20) -> pd.DataFrame:
    """
        ...
        if rag_enabled and results and results.get('agent_files'):
            ...
            if combined_file and os.path.exists(combined_file):
                ...
                try:
                    ...
                    output_path = str(combined_path_obj.parent / f"{combined_path_obj.stem}_with_reasoning{combined_path_obj.suffix}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate investment reasoning for stocks using RAG or run pipeline with reasoning")
    parser.add_argument(
        "--mode",
    ...
    df_enhanced = process_csv_with_rag(df, chain, sample_size=args.sample_size, max_workers=args.max_workers, main_pbar=main_pbar)
