...
class QuantamentalTrainer:
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"   Label distribution: {df_model['label'].value_counts(normalize=True).to_dict()}")
        
        return df_model
    
    def create_train_test_split(self, df: pd.DataFrame, 
                               test_year: int = 2025, 
                               test_month: int = 10) -> tuple:
        """
        ...
        logger.info(f"   Train: {train_start.date()} → {train_end.date()} ({len(df_train):,} rows)")
    ...
    def evaluate_model(self, model, scaler, X_test: pd.DataFrame, 
    ...
    def find_optimal_threshold(self, model, scaler, X_train: pd.DataFrame, 
                              y_train: pd.Series) -> float:
        """
        Find optimal probability threshold using precision-recall curve
        """
        X_train_scaled = scaler.transform(X_train.replace([np.inf, -np.inf], 0).fillna(0))
    ...
    def create_plots(self, model, y_test, y_pred, y_prob, 
    ...
    def train_with_wandb(self, df: pd.DataFrame, 
                        test_year: int = 2025, 
                        test_month: int = 10) -> tuple:
        """
        ...
        # Train/test split
        df_train, df_test = self.create_train_test_split(df_model, test_year, test_month)
        ...
        train_data_path = f"{self.data_dir}/train_data_{test_year}{test_month:02d}.parquet"
        ...
        logger.info(f"Training data logged to W&B as artifact (version will auto-increment)")
