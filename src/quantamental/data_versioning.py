...
class DataVersionManager:
    ...
    def __init__(self, config: dict = None):
        ...
        if HAS_GCS:
            try:
                ...
                bucket_name = config.get('gcs', {}).get('bucket_name', 'stock-busters-data')
    ...
    def save_input_data_snapshot(self, 
                                 ohlcv_path: str,
                                 sp500_path: str,
                                 fundamentals_path: str = None,
                                 version_tag: str = None) -> Dict:
        """
        ...
        # Version each file
        files_to_version = {
            'ohlcv_raw': ohlcv_path,
            'sp500_index': sp500_path
        }
                ...
                try:
                    ...
                    if df.index.name == 'date' or 'date' in df.columns:
                        ...
                        metadata['date_range'] = {
                            ...
                            'days': (pd.to_datetime(date_col).max() - pd.to_datetime(date_col).min()).days
                        }
                    ...
                    logger.info(f"    {name}: {len(df):,} rows, {len(df.columns)} columns, {metadata['file_size_mb']} MB")
        ...
        for name, path in files_to_version.items():
            ...
            try:
                ...
                blob.metadata = {
                    ...
                    'file_size_mb': str(round(Path(path).stat().st_size / 1024 / 1024, 2))
                }
                ...
                metadata_blob = self.gcs_bucket.blob(f"metadata/{name}_{version_tag}.json")
    ...
    def create_version_snapshot(self, version_tag: str = "ms4_submission") -> Dict:
        ...
        for name, path in input_files.items():
            ...
            try:
                ...
                logger.info(f"   ✅ {name}: {len(df):,} rows, {metadata['file_size_mb']} MB → v{logged_artifact.version}")
        ...
        for name, path in output_files.items():
            ...
            try:
                ...
                logger.info(f"   ✅ {name}: {len(df):,} rows, {len(df.columns)} cols → v{logged_artifact.version}")
        ...
        run.summary['artifacts_created'] = artifacts_created
        run.summary['version_tag'] = version_tag
        run.summary['num_input_files'] = len([v for v in version_info['input_files'].values() if 'error' not in v])
        run.summary['num_output_files'] = len([v for v in version_info['output_files'].values() if 'error' not in v])
        ...
        logger.info(f"   Input files versioned: {len([v for v in version_info['input_files'].values() if 'error' not in v])}")
        logger.info(f"   Output files versioned: {len([v for v in version_info['output_files'].values() if 'error' not in v])}")
