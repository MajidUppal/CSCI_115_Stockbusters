...
def calculate_hybrid_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ============================================
    tech_cols = [
        "return_1m", "ema_12", "ema_26", "macd", "macd_signal", 
        "macd_hist", "RSI_14", "volatility_21d"
    ]
    
    fund_cols = [
        "payoutRatio", "cashPerShare", "dividendYield", "revenuePerShare", 
        "earningsYield", "evToOperatingCashFlow", "netDebtToEBITDA", 
        "freeCashFlowYield", "pocfratio", "intangiblesToTotalAssets", 
        "netIncomePerShare", "incomeQuality", "capexToDepreciation",
        "operatingCashFlowPerShare", "peRatio", "returnOnTangibleAssets", 
        "investedCapital", "roe", "roic", "debtToEquity", 
    ...
    logger.info(f"    Fundamental_Score: {df_score['Fundamental_Score'].mean():.3f} avg")
    ...
    logger.info(f"      {(df_score['H_Score Recommendation'].str.contains('Buy')).sum()} Buy signals")
    logger.info(f"      {(df_score['H_Score Recommendation'] == 'Hold / Neutral').sum()} Hold")
    logger.info(f"      {(df_score['H_Score Recommendation'] == 'Avoid / Bearish').sum()} Avoid")
    
    # ============================================
    # Step 7: Calculate Hybrid_Rank
    # ============================================
    # Rank within each month
    df_score["Hybrid_Rank"] = (
        df_score
        .groupby("ym")["Hybrid_Score"]
...
def calculate_backtest_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'fwd_return_1m' not in df.columns:
        logger.warning("     'fwd_return_1m' not found - cannot calculate backtest metrics")
        # Add NaN columns
        for col in ['n_periods', 'avg_fwd_1m_ret', 'vol_1m', 'sharpe_1m_annual',
                    'max_drawdown', 'hit_rate_pos', 'hit_rate_vs_sp500', 'cagr']:
            df[col] = np.nan
        return df
    ...
    if len(use) == 0:
        ...
        logger.warning("    This is normal for latest month predictions (no future data yet)")
    ...
    metrics = use.groupby("symbol", as_index=False).apply(_metrics, include_groups=False).reset_index()
    ...
    if len(metrics) > 0 and 'sharpe_1m_annual' in metrics.columns:
        ...
        logger.warning(f"      No valid backtest metrics (this is normal for latest predictions)")
