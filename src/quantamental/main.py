...
def run_data_collection(config, force_refresh=False):
    ...
    if force_refresh:
        ...
        for f in ['ohlcv_raw.parquet', 'fundamentals_combined.parquet', 'sp500_index.parquet']:
            path = f"{data_dir}/{f}"
...
def run_data_processing(config, data=None):
    ...
    return processor.process_all(data['ohlcv'], data['fundamentals'], data['sp500_index'])
    ...
    if use_validated_model and HAS_MODEL_VALIDATION:
        try:
            ...
            if model is not None:
                log.info(f"Using validated model: {status} ({metrics.get('accuracy', 0):.1%})")
...
def run_rag_reasoning(config, combined_csv_path=None, sample_size=None):
    ...
    if combined_csv_path is None:
        data_dir = config['data']['data_dir']
        combined_csv_path = f"{data_dir}/combined_quantamental_hybrid_with_factors_and_backtest.csv"
    ...
    try:
        enhanced_path = add_reasoning_to_combined_file(
            ...
            gcs_path=config.get('gcs', {}).get('output_folder', 'model_output') + '/combined_with_reasoning.csv'
        )
...
def main():
    ...
    parser.add_argument('--step', 
                        choices=['all', 'collect', 'process', 'train', 'predict', 'backtest', 'rag', 'version'],
                        default='all')
    parser.add_argument('--force-refresh', action='store_true', help='Re-fetch data from API')
    parser.add_argument('--skip-training', action='store_true', help='Use existing model')
    parser.add_argument('--enable-rag', action='store_true', help='Add RAG reasoning')
    parser.add_argument('--rag-sample', type=int, default=None, help='Limit RAG to N stocks')
    parser.add_argument('--no-version', action='store_true', help='Skip data versioning')
