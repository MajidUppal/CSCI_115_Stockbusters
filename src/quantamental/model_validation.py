...
def get_best_model(project=WANDB_PROJECT, allow_degraded=True):
    ...
    try:
        ...
        for art in artifacts:
            ...
            if acc == 0:
                log.warning(f"{art.version}: no metadata, using fallback {FALLBACK_ACC:.0%}")
                acc = FALLBACK_ACC
                roc = 0.45
        ...
        log.info(f"Found {len(models)} models, best: {best['version']} ({best['accuracy']:.1%})")
        ...
        if best['accuracy'] >= PROD_THRESHOLD:
            ...
            status = 'degraded'
            log.warning(f"Degraded model: {best['accuracy']:.1%} (need {PROD_THRESHOLD:.0%} for prod)")
        else:
            log.error(f"No valid model. Best: {best['accuracy']:.1%}, need: {MIN_THRESHOLD:.0%}")
            return None, {}, 'rejected'
...
class ModelValidator:
    ...
    def __init__(self, config=None, allow_degraded=True):
        ...
        if self.status == 'rejected':
            raise ValueError(f"No valid model found (need {MIN_THRESHOLD:.0%}+ accuracy)")
