...
class DataProcessor:
    ...
    def compute_technicals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Returns
        df["return_1m"] = g["close"].transform(lambda x: x.pct_change(21, fill_method=None))
        df["return_1d"] = g["close"].transform(lambda x: x.pct_change(1, fill_method=None))
    ...
    def merge_fundamentals(self, ohlcv_ta: pd.DataFrame, 
        ...
        for sym in tqdm(symbols, desc="Merging"):
            ...
            m = pd.merge_asof(left, right, on="date", 
        ...
        logger.info(f"✅ Merged data: {len(merged):,} rows, {merged['symbol'].nunique()} symbols")
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df[self.fund_cols] = df.groupby("symbol")[self.fund_cols].ffill(limit=forward_limit)
        
        # Optional: backfill once at the start
        backfill_limit = self.config['processing']['backfill_limit']
        df[self.fund_cols] = df.groupby("symbol")[self.fund_cols].bfill(limit=backfill_limit)
        ...
        logger.info(f"✅ Cleaned: {len(df):,} rows ({initial_rows-len(df):,} removed), "
    ...
    def validate_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            plt.close()
            logger.info(f"   Saved coverage plot → {self.data_dir}/coverage_distribution.png")
        ...
        logger.info(f"✅ Final dataset: {df['symbol'].nunique()} symbols, {len(df):,} rows")
    ...
    def process_all(self, ohlcv: pd.DataFrame, fundamentals: pd.DataFrame, 
                   sp500_index: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        validated.to_parquet(f"{self.data_dir}/quantamental_filtered.parquet", index=False)
        ...
        validated = pd.merge(validated, sp500_index[["date", "sp500_close"]], 
                           on="date", how="left")
        ...
        df_monthly.to_parquet(f"{self.data_dir}/quantamental_monthly.parquet", index=False)
...
def main():
    ...
    fundamentals = pd.read_parquet(f"{config['data']['data_dir']}/fundamentals_combined.parquet")
