...
class FMPDataCollector:
    ...
    async def fetch_ohlcv_symbol(self, sess: aiohttp.ClientSession, 
        ...
        return df[["symbol", "date", "open", "high", "low", "close", 
                  "adjClose", "volume"]].rename(columns={"adjClose": "adj_close"})
    ...
    async def fetch_fundamentals_symbol(self, sess: aiohttp.ClientSession, 
                                       symbol: str, period: str = "quarter") -> pd.DataFrame:
    ...
    async def fetch_sp500_index(self) -> pd.DataFrame:
        ...
        url = (f"{self.base_url}/historical-price-full/%5EGSPC"
               f"?from={self.start_date}&to={self.end_date}&apikey={self.api_key}")
    ...
    async def fetch_company_profile(self, sess: aiohttp.ClientSession, 
                                    symbol: str) -> dict:
