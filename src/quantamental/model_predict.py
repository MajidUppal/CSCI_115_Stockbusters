...
class QuantamentalPredictor:
    ...
    def load_model_from_wandb(self, artifact_name: str = "quantamental-model:latest") -> tuple:
        """
        ...
        artifact = api.artifact(
            f"{self.config['wandb']['project']}/{artifact_name}",
            type='model'
        )
    ...
    def prepare_prediction_data(self, df: pd.DataFrame, 
                               predict_month: pd.Timestamp = None) -> pd.DataFrame:
        """
        ...
        if predict_month is None:
            ...
            df_predict = df[df["date"].dt.to_period("M") == predict_month.to_period("M")].copy()
    ...
    def save_predictions(self, df_predict: pd.DataFrame, 
        ...
        output_df = df_predict[["symbol", "date", "close", "pred_prob", "pred_rank"]].copy()
