...
def get_company_profile(ticker, df_company_profile):
    try:
        company_profile = df_company_profile[df_company_profile['symbol']==ticker.upper()].fillna('NaN').to_dict(orient='records')[0]
...
def user_pref_stock_selection(df, user_pref):
    ...
    if (user_pref['long_term'] & user_pref['short_term']) or ((not user_pref['long_term']) & (not user_pref['short_term'])):
        ...
        recommended_model_filter = df['H_Score Recommendation'].isin(['Long-Term Buy (Fundamental)', 'Short-Term Buy (Momentum)'])
        print("long_short")
    ...
    # print(risk_filter)
    df_shortlisted = df[recommended_model_filter & recommended_model_filter & risk_filter].sort_values(by=long_short_crit, ascending=False)
