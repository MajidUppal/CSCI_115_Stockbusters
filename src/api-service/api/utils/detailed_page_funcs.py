...
def get_company_profile(ticker, df_company_profile):
    try:
        company_profile = (
            df_company_profile[df_company_profile["symbol"] == ticker.upper()]
            .fillna("NaN")
...
def get_quant_data(ticker, df_quant_model):
    try:
        quant_data = (
            df_quant_model[df_quant_model["symbol"] == ticker.upper()]
            .fillna("NaN")
            .to_dict(orient="records")[0]
        )
        return quant_data
...
def user_pref_stock_selection(df, user_pref):
    ...
    if (user_pref["long_term"] & user_pref["short_term"]) or (
        (not user_pref["long_term"]) & (not user_pref["short_term"])
    ):
    ...
    if (user_pref["low_risk"] & user_pref["high_risk"]) or (
        (not user_pref["low_risk"]) & (not user_pref["high_risk"])
    ):
    ...
    # print(risk_filter)
    df_shortlisted = df[
        recommended_model_filter & recommended_model_filter & risk_filter
    ].sort_values(by=long_short_crit, ascending=False)
