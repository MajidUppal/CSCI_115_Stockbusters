...
class TestChatbotRouterUnit:
    ...
    def setup(self):
        ...
        with patch("api.routers.chatbot_final.service_account") as mock_sa, \
            ...
            mock_agent.graph.invoke.return_value = {
                'messages': [MagicMock(content="Test AI response")],
                'user_pref': {}
            }
    ...
    def test_continue_chat_success(self):
        ...
        self.mock_agent.graph.invoke.return_value = {
            'messages': [MagicMock(content="How can I help?")],
            'user_pref': {}
        }
        
        response = self.client.post(
            f"/gemini/chats/{test_chat_id}",
            json={"message": "I prefer long term"},
            headers={"X-Session-ID": "test-session"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_continue_chat_with_user_pref_update(self):
        ...
        response = self.client.post(
            f"/gemini/chats/{test_chat_id}",
            json={"message": "Long term, low risk"},
            headers={"X-Session-ID": "test-session"}
        )
        assert response.status_code == 200
...
class TestStockDetailsRouterUnit:
    ...
    def test_generate_report_handles_empty_recommendations(self):
        ...
        empty_df = pd.DataFrame(columns=['symbol', 'Hybrid_Score', 'H_Score Recommendation', 
                                        'volatility_21d', 'max_drawdown', 'sector'])
